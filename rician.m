...
Kfactor = 100;        % Rician fading channel K factor
...
ricianchan = comm.RicianChannel('KFactor',Kfactor); % Create Rayleigh fading channel
...
for ii = 1:length(EbNo)
    ...
    % Add Rician fading
    fadedSignal = ricianchan(receivedSignal);
...
theory_ber = berfading(EbNo, 'qam', M, 1, Kfactor); % theoretical Rician BER
...
axis([0 16 10^-8 0.5])
grid on
legend('Theoretical AWGN', 'Theoretical Rician', 'Simulated');
xlabel('Eb/N0 (dB)');
ylabel('BER');
title(['BER vs SNR for ', num2str(M), '-QAM in Rician']);